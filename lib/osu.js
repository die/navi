...
class OsuAPI {
    ...
    async getPlayer(user, mode) {
        const config = await this.#getConfig(
            'get',
            this.#baseUrl + `/users/${user}/${mode ?? 'osu'}`,
            true
        );
        try {
            ...
            await db.execute(
                `INSERT INTO osu_ids (id, username) VALUES (?, ?) ON DUPLICATE KEY UPDATE username = ?`,
    ...
    async getRecentScore(user, mode) {
        const config = await this.#getConfig(
            'get',
            this.#baseUrl +
                `/users/${user}/scores/recent?include_fails=1&mode=${mode}`,
            true
        );
        try {
    ...
    async getBestScores(user, mode) {
        const config = await this.#getConfig(
            'get',
            this.#baseUrl + `/users/${user}/scores/best?limit=100&mode=${mode}`,
            true
        );
        try {
            const response = await axios.request(config);
    ...
    async #getAccessToken() {
        const [rows] = await db.execute(
            'SELECT access_token, refresh_token, expiry, created_at FROM api_tokens WHERE token_id = ?',
            ['osu']
        );
        ...
        if (timeTimestamp + expiry < nowTimestamp) {
            ...
            await db.execute(
                'UPDATE api_tokens SET access_token = ?, refresh_token = ?, expiry = ?, created_at = NOW() WHERE token_id = ?',
                [new_access_token, new_refresh_token, new_expiry, 'osu']
            );
    ...
    simulateOsu(options) {
        ...
        const percent_combo = options.percent_combo
            ? `-C ${options.percent_combo}`
            : '';
        const mods = options.mods
            ? options.mods.map((mod) => `-m ${mod}`).join(' ')
            : '';
    ...
    simulateCatch(options) {
        ...
        const mods = options.mods
            ...
            : '';
        const misses = options.misses ? `-X ${options.misses}` : '';
        const tinyDroplets = options.tinyDroplets
            ? `-T ${options.tinyDroplets}`
            : '';
...
class GameMode {
    ...
    static getGameModesAsChoices() {
        return Object.keys(GameMode).map((mode) => ({
            name: GameMode[mode],
            value: mode,
        }));
    }
