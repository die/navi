...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('phasdecrypt')
		.setDescription('Decypt a Phasmophobia save file often located at %appdata%\\LocalLow\\Kinetic Games\\Phasmophobia')
	...
	async execute(interaction) {
		...
		const response = await fetch(attachment.attachment).catch(err => { return interaction.reply(err); });
		const buffer = new Uint8Array(await response.arrayBuffer());
		const iv = buffer.slice(0, 16);
		const key = crypto.pbkdf2Sync('t36gref9u84y7f43g', iv, 100, 16, 'sha1');
		const decipher = crypto.createDecipheriv('aes-128-cbc', key, iv);
		const finalBuffer = Buffer.concat([decipher.update(buffer.slice(16)), decipher.final()]);
		const json = JSON.parse(finalBuffer.toString().replace(/(\d+):(\d+)/g, '"$1":"$2"'));
		await interaction.deferReply();
		function displaySecondsAsHours(seconds) {
		...
		const xpUntilLevelUp = xp[newLevel - 1].value - json['Experience'].value;
		const levelStats = {
		...
		const levelEmbed = new EmbedBuilder()
			...
			.addFields(
				{ name: 'Ghost Type', value: `${json['GhostType'].value}`, inline: false },
				{ name: labels[0], value: `${Object.keys(levelStats).map(k => `${k}: \`${levelStats[k]}\``).join('\n')}`, inline: false },
			)
			.setTimestamp();
