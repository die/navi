    ...
    async execute(interaction, db, emojis) {
        ...
        if (!bestScores || bestScores.length == 0)
            return await interaction.reply(
                `Failed to find best scores for ${playerId}`
            );
        ...
        const embed = new EmbedBuilder()
            .setAuthor({
                name: `Best Plays for ${firstScore.user.username}`,
                iconURL: `${firstScore.user.avatar_url}`,
                url: `https://osu.ppy.sh/users/${firstScore.user.id}`,
            })
            .setThumbnail(`${firstScore.user.avatar_url}`)
            .setTimestamp();

        async function generateFunction(e, currentPage, length) {
            const pageScores = sorted.slice(
                currentPage * length,
            ...
            const description = pageScores
                .map((score) => {
                    ...
                    const rankEmoji = emojis.find(
                        (emoji) => emoji.name == `ranking_${score.rank}`
                    );
                    ...
                    const ifFc =
                        score.statistics.count_miss == 0
                            ? ''
                            : `*(${simulation.performance_attributes.pp.toFixed(2)} pp for ${simulation.score.accuracy.toFixed(2)}% FC)*`;
            ...
            e.setFooter({
                text: `Page ${currentPage + 1}/${Math.floor(bestScores.length / length)}`,
            });
            return e;
        }
