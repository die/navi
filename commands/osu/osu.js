...
module.exports = {
    ...
    async execute(interaction, db, emojis) {
        ...
        const joinDate = new Date(player.join_date).toLocaleString('en-US', {
            year: 'numeric',
            month: 'long',
            day: '2-digit',
            hour: 'numeric',
        ...
        const playstyles = player.playstyle
            ? player.playstyle.map((p) => `\`${p}\``).join(', ')
            : '`N/A`';
        const ranking_xh_emoji = emojis.find(
            (emoji) => emoji.name == 'ranking_XH'
        );
        const ranking_x_emoji = emojis.find(
            (emoji) => emoji.name == 'ranking_X'
        );
        const ranking_sh_emoji = emojis.find(
            (emoji) => emoji.name == 'ranking_SH'
        );
        const ranking_s_emoji = emojis.find(
            (emoji) => emoji.name == 'ranking_S'
        );
        const ranking_a_emoji = emojis.find(
            (emoji) => emoji.name == 'ranking_A'
        );
        ...
        const embed = new EmbedBuilder()
            .setAuthor({
                name: `${player.username}'s ${GameMode.displayGameMode(mode)} Profile`,
